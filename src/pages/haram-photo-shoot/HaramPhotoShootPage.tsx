...
export function HaramPhotoShootPage() {
  ...
  const currentFrameSrc = HARAM_FRAME_SRC[Math.min(nextSlot, 3)];
  const statusLabel =
    nextSlot >= 4
      ? "촬영 완료 (4/4)"
  ...
  const captureFrame = useCallback(async (slotIndex: number) => {
    ...
    try {
      const frameImage = await loadImage(
        HARAM_FRAME_SRC[Math.min(slotIndex, HARAM_FRAME_SRC.length - 1)]
      );
  ...
  useEffect(() => {
    ...
    navigate("/haram-photo-shoot/result", { replace: true });
