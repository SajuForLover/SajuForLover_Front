...
export function NayeonPhotoShootPage() {
  ...
  const currentFrameSrc = NAYEON_FRAME_SRC[Math.min(nextSlot, 3)];
  const statusLabel =
    nextSlot >= 4
      ? "촬영 완료 (4/4)"
  ...
  const captureFrame = useCallback(async (slotIndex: number) => {
    ...
    try {
      const frameImage = await loadImage(
        NAYEON_FRAME_SRC[Math.min(slotIndex, NAYEON_FRAME_SRC.length - 1)]
      );
  ...
  useEffect(() => {
    ...
    navigate("/nayeon-photo-shoot/result", { replace: true });
