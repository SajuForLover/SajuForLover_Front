...
export function ChanyoungPhotoShootPage() {
  ...
  const currentFrameSrc = CHANYOUNG_FRAME_SRC[Math.min(nextSlot, 3)];
  const statusLabel =
    nextSlot >= 4
      ? "촬영 완료 (4/4)"
  ...
  const captureFrame = useCallback(async (slotIndex: number) => {
    ...
    try {
      const frameImage = await loadImage(
        CHANYOUNG_FRAME_SRC[Math.min(slotIndex, CHANYOUNG_FRAME_SRC.length - 1)]
      );
